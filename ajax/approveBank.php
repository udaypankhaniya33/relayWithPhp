...
if (mysqli_num_rows($user) > 0) {
    $update = mysqli_query($connection, "UPDATE  `user` SET `bankDetailVerification`='2' WHERE `userId`='" .base64_decode( $_POST["CustomerId"]) . "' ");
    if ($update) {
        $user = mysqli_fetch_array(mysqli_query($connection, "SELECT * FROM    `user`  WHERE `userId`='" .base64_decode( $_POST["CustomerId"]) . "' "));
        $message = "Hi " . $user["fullName"] . " ,  Your Request of Bank Details Verification has been Approved, Best Wishes From Team Relay.";
